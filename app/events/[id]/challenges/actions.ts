  ...
  if (loser?.email) {
    await sendEmail({
      ...
      html: `
        ...
        <p>Once you've settled up, log in and mark it complete.</p>
      `,
    })
  }
...
export async function markCompleted(challengeId: string) {
  ...
  const { data: challenge } = await supabase
    .from('challenges')
    .select('event_id, winner_id, challenger_id, challenged_id, status')
    .eq('id', challengeId)
    .single()

  if (!challenge) return { error: 'Challenge not found' }

  if (!challenge.winner_id) return { error: 'Result not set yet' }
  if (!['result_set', 'awaiting_witness'].includes(challenge.status)) {
    return { error: 'Challenge is not ready to settle yet' }
  }

  const participantIds = [challenge.challenger_id, challenge.challenged_id]
  if (!participantIds.includes(user.id)) {
    return { error: 'Only challenge participants can mark as settled' }
  }
